...
$(document).ready(function(){
    ...
    // var aid = search_url.split('&')[0].split('=')[1]
    // var sd = search_url.split('&')[2].split('=')[1]
    // var ed = search_url.split('&')[3].split('=')[1]

    $.get('/house/my_search/' + search_url, function (data) {

        var search_html = template('search_house_script', {ohouse:data.house_dict});
        $('.house-list').html(search_html)
    });

    $.get('/house/area_facility/', function (data) {

        var areas_script = template('areas_script_id', {areas:data.areas});
        $('.filter-area').html(areas_script)
